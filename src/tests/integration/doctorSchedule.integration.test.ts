...
describe('agenda canónica del doctor con PostgreSQL real', () => {
  ...
  beforeEach(async () => {
    ...
    const [doctorUser, patientUser, clinicUser, otherClinicUser] = await Promise.all([
      prisma.user.create({ data: { email: 'schedule.doctor@zenda.test', firstName: 'Agenda', lastName: 'Doctor', passwordHash, role: 'DOCTOR' } }),
      prisma.user.create({ data: { email: 'schedule.patient@zenda.test', firstName: 'Agenda', lastName: 'Paciente', passwordHash, role: 'PATIENT' } }),
      prisma.user.create({ data: { email: 'schedule.clinic@zenda.test', firstName: 'Agenda', lastName: 'Clínica', passwordHash, role: 'CLINIC_ADMIN' } }),
      prisma.user.create({ data: { email: 'schedule.other-clinic@zenda.test', firstName: 'Otra', lastName: 'Clínica', passwordHash, role: 'CLINIC_ADMIN' } }),
    ]);
  ...
  it('rechaza paciente inexistente, servicio ajeno, clínica ajena, fuera de jornada y sobre bloqueo', async () => {
    ...
    const otherDoctorUser = await prisma.user.create({
      data: { email: 'schedule.other-doctor@zenda.test', firstName: 'Otro', lastName: 'Doctor', passwordHash: 'x', role: 'DOCTOR' },
    });

...
import { setEmailVerificationAdapterForTests, setPatientInvitationEmailAdapterForTests } from '../../services/email.service';

describe('citas manuales con paciente invitado y PostgreSQL real', () => {
  let doctorToken = ''; let clinicId = ''; let serviceId = '';
  const invitations: Array<{ to: string; token: string }> = [];
  const verificationTokens: string[] = [];

  beforeEach(async () => {
    assertIntegrationDatabase(); await clearIntegrationDatabase(); invitations.length = 0; verificationTokens.length = 0;
    setPatientInvitationEmailAdapterForTests(async email => { invitations.push({ to: email.to, token: email.token }); });
  ...
  afterAll(async () => { setPatientInvitationEmailAdapterForTests(undefined); setEmailVerificationAdapterForTests(undefined); await clearIntegrationDatabase(); await disconnectPrisma(); });
  ...
  it('no crea un usuario fantasma, invita, verifica y reclama exactamente una vez', async () => {
    ...
    expect(invitations).toEqual([expect.objectContaining({ to: 'maria.invitada@zenda.test' })]);
  ...
  it('no expone el token de invitación si el entorno es production', async () => {
    ...
    try {
      ...
      expect(JSON.stringify(response.body)).not.toContain(invitations[0]?.token || '__no_token__');

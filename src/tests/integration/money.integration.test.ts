...
describe('snapshot monetario con PostgreSQL real', () => {
  ...
  beforeEach(async () => {
    ...
    const [doctorUser, clinicUser, patient] = await Promise.all([
      prisma.user.create({ data: { email: 'doctor.money@zenda.test', emailNormalized: 'doctor.money@zenda.test', firstName: 'Doc', lastName: 'Money', passwordHash: await bcrypt.hash('password-123456', 4), role: 'DOCTOR' } }),
      prisma.user.create({ data: { email: 'clinic.money@zenda.test', emailNormalized: 'clinic.money@zenda.test', firstName: 'Clinic', lastName: 'Money', passwordHash: 'x', role: 'CLINIC_ADMIN' } }),
      prisma.user.create({ data: { email: 'patient.money@zenda.test', emailNormalized: 'patient.money@zenda.test', firstName: 'Patient', lastName: 'Money', passwordHash: 'x', role: 'PATIENT' } }),
    ]);
  ...
  it('persiste $35.50 en 3550, crea snapshot y no acepta precio o duración del cliente', async () => {
    ...
    const booking = await request(app).post('/api/bookings/book').set('Authorization', `Bearer ${patientToken}`)
      .send({ doctorId, clinicId, serviceId: service.body.id, date: '2099-08-01', startTime: '09:00', paymentMethod: 'CASH', price: 1, duration: 1 }).expect(201);
  ...
  it('rechaza reservar un servicio de otro médico', async () => {
    const otherUser = await prisma.user.create({ data: { email: 'other.money@zenda.test', emailNormalized: 'other.money@zenda.test', firstName: 'Other', lastName: 'Doc', passwordHash: 'x', role: 'DOCTOR' } });
    ...
    await request(app).post('/api/bookings/book').set('Authorization', `Bearer ${patientToken}`)
      .send({ doctorId, clinicId, serviceId: service.id, date: '2099-08-02', startTime: '09:00', paymentMethod: 'CASH' }).expect(403);
  });
  ...
  it('permite una sola reserva simultánea para el mismo intervalo', async () => {
    ...
    const body = { doctorId, clinicId, serviceId: service.id, startsAt: '2099-08-03T09:00:00-05:00', paymentMethod: 'CASH' };
    ...
    expect(await prisma.appointment.count({ where: { doctorProfileId: doctorId, startsAt: new Date('2099-08-03T14:00:00.000Z') } })).toBe(1);
  });
  ...
  it('cancela y reprograma con historial real sin alterar snapshots', async () => {
    ...
    const created = await request(app).post('/api/bookings/book').set('Authorization', `Bearer ${patientToken}`).send({ doctorId, clinicId, serviceId: service.id, startsAt: '2099-08-04T09:00:00-05:00', paymentMethod: 'CASH' }).expect(201);
    const moved = await request(app).patch(`/api/bookings/${created.body.id}/reschedule`).set('Authorization', `Bearer ${patientToken}`).send({ startsAt: '2099-08-04T10:00:00-05:00' }).expect(200);
  ...
  it('reserva y bloqueo simultáneos dejan una sola ocupación', async () => {
    ...
    const startsAt = '2099-08-05T09:00:00-05:00';
    const [booking, block] = await Promise.all([
      ...
      request(app).post('/api/schedule-blocks').set('Authorization', `Bearer ${doctorToken}`).send({ doctorId, clinicId, startsAt, endsAt: '2099-08-05T09:30:00-05:00', reason: 'Prueba de carrera' }),
    ]);
    expect([booking.status, block.status].sort()).toEqual([201, 409]);
    const instant = new Date('2099-08-05T14:00:00.000Z');
  ...
  it('reprogramación y reserva simultáneas tienen un único ganador en el horario B', async () => {
    ...
    const original = await request(app).post('/api/bookings/book').set('Authorization', `Bearer ${patientToken}`).send({ doctorId, clinicId, serviceId: service.id, startsAt: '2099-08-06T09:00:00-05:00', paymentMethod: 'CASH' }).expect(201);
    const target = '2099-08-06T10:00:00-05:00';
    ...
    expect(await prisma.appointment.count({ where: { doctorProfileId: doctorId, startsAt: new Date('2099-08-06T15:00:00.000Z') } })).toBe(1);
    ...
    if (rescheduled.status === 409) expect((await prisma.appointment.findUniqueOrThrow({ where: { id: original.body.id } })).startsAt).toEqual(new Date('2099-08-06T14:00:00.000Z'));
  });
  ...
  it('dos reprogramaciones simultáneas dejan un único historial ganador', async () => {
    ...
    const original = await request(app).post('/api/bookings/book').set('Authorization', `Bearer ${patientToken}`).send({ doctorId, clinicId, serviceId: service.id, startsAt: '2099-08-07T09:00:00-05:00', paymentMethod: 'CASH' }).expect(201);
    const [toB, toC] = await Promise.all([
      request(app).patch(`/api/bookings/${original.body.id}/reschedule`).set('Authorization', `Bearer ${patientToken}`).send({ startsAt: '2099-08-07T10:00:00-05:00' }),
      request(app).patch(`/api/bookings/${original.body.id}/reschedule`).set('Authorization', `Bearer ${patientToken}`).send({ startsAt: '2099-08-07T11:00:00-05:00' }),
    ]);
    ...
    expect([new Date('2099-08-07T15:00:00.000Z').getTime(), new Date('2099-08-07T16:00:00.000Z').getTime()]).toContain(final.startsAt?.getTime());
  ...
  it('confirma al paciente sin cambiar el pago y deriva etiqueta de calendario', async () => {
    ...
    const created = await request(app).post('/api/bookings/book').set('Authorization', `Bearer ${patientToken}`).send({ doctorId, clinicId, serviceId: service.id, startsAt: '2099-08-08T09:00:00-05:00', paymentMethod: 'CASH' }).expect(201);
  ...
  it('ejecuta check-in, turno, atención, complete y reseña verificada', async () => {
    ...
    const created = await request(app).post('/api/bookings/book').set('Authorization', `Bearer ${patientToken}`).send({ doctorId, clinicId, serviceId: service.id, startsAt: '2099-08-09T09:00:00-05:00', paymentMethod: 'CASH' }).expect(201);
  ...
  it('no-show marca cita y turno como inasistencia y no habilita reseña', async () => {
    ...
    const created = await request(app).post('/api/bookings/book').set('Authorization', `Bearer ${patientToken}`).send({ doctorId, clinicId, serviceId: service.id, startsAt: '2099-08-10T09:00:00-05:00', paymentMethod: 'CASH' }).expect(201);
  ...
  it('las rutas legacy de asistente delegan el ciclo operativo, se deprecian y mantienen autorización', async () => {
    ...
    const assistantUser = await prisma.user.create({ data: { email: 'assistant.money@zenda.test', emailNormalized: 'assistant.money@zenda.test', firstName: 'Assistant', lastName: 'Money', passwordHash: 'x', role: 'ASSISTANT' } });
    ...
    const created = await request(app).post('/api/bookings/book').set('Authorization', `Bearer ${patientToken}`).send({ doctorId, clinicId, serviceId: service.id, startsAt: '2099-08-11T09:00:00-05:00', paymentMethod: 'CASH' }).expect(201);
    await request(app).patch(`/api/bookings/${created.body.id}/confirm`).set('Authorization', `Bearer ${patientToken}`).send({}).expect(200);
    ...
    const outsider = await prisma.user.create({ data: { email: 'outsider.money@zenda.test', emailNormalized: 'outsider.money@zenda.test', firstName: 'Out', lastName: 'Sider', passwordHash: 'x', role: 'DOCTOR' } });
  ...
  it('legacy missed delega a no-show y una reseña sigue prohibida', async () => {
    ...
    const assistantUser = await prisma.user.create({ data: { email: 'assistant.missed@zenda.test', emailNormalized: 'assistant.missed@zenda.test', firstName: 'Assistant', lastName: 'Missed', passwordHash: 'x', role: 'ASSISTANT' } });
    await prisma.assistantProfile.create({ data: { userId: assistantUser.id, doctorProfileId: doctorId } });
    const created = await request(app).post('/api/bookings/book').set('Authorization', `Bearer ${patientToken}`).send({ doctorId, clinicId, serviceId: service.id, startsAt: '2099-08-12T09:00:00-05:00', paymentMethod: 'CASH' }).expect(201);
  ...
  it('confirmación legacy y efectivo heredado se mantienen separados del estado clínico', async () => {
    ...
    const created = await request(app).post('/api/bookings/book').set('Authorization', `Bearer ${patientToken}`).send({ doctorId, clinicId, serviceId: service.id, startsAt: '2099-08-13T09:00:00-05:00', paymentMethod: 'CASH' }).expect(201);

...
function paymentRow(payment: FinancePayment) {
  ...
  return { paymentId: payment.id, date: appointment.startsAt?.toISOString() || null, patient: { id: appointment.patient.id, displayName: `${appointment.patient.firstName} ${appointment.patient.lastName}`.trim() }, doctor: { id: appointment.doctorProfile.id, displayName: `${appointment.doctorProfile.user.firstName} ${appointment.doctorProfile.user.lastName}`.trim() }, clinic: { id: appointment.clinicProfile.id, name: appointment.clinicProfile.name }, service: { id: appointment.service.id, name: appointment.serviceNameSnapshot || appointment.service.name }, amountCents: payment.amountCents, amount: cashAmountFromCents(payment.amountCents), currency: payment.currency, status: payment.status, method: payment.method, confirmedBy: payment.confirmedBy ? { id: payment.confirmedBy.id, displayName: `${payment.confirmedBy.firstName} ${payment.confirmedBy.lastName}`.trim() } : null, confirmedAt: payment.confirmedAt?.toISOString() || null, requiresReview: payment.requiresReview };
}

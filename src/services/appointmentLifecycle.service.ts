...
/** Compatibility bridge for /api/assistant/:id/postpone. The canonical resource is a turn. */
export async function postponeAppointmentTurn(appointmentId: string, actor: Actor) { const appointment = await loadAuthorized(appointmentId, actor); if (['CANCELLED', 'COMPLETED', 'MISSED'].includes(appointment.status)) throw new BookingError('TURN_NOT_ALLOWED', 422, 'No se puede posponer el turno de esta cita.'); const turn = await getOrCreateAppointmentTurn(appointmentId, actor); return delayTurn(turn.id, actor); }
